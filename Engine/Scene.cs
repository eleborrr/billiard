...
namespace Billiards.Game
{
    class Scene : IScene //сцена
    {
        ...
        {            
            ...
            int addIndex = 1;
            Random r = new Random();            
            for (int i = 0; i<5;i++) //сложная математическая штука которая выстраивает пирамиду
            {
        ...
        public void HandleBallCollision(Ball ball1, Ball ball2) 
        {
            ...
            var newV1 = (v1 + v2 - (v1 - v2)) / 2;  // какая то формула которую нашли в интернет
            var newV2 = (v1 + v2 - (v2 - v1)) / 2;

            ball1.AddToSpeed(direction * (newV1 - v1));  // сложной математикой считаем скорость при столкновении
            ball2.AddToSpeed(direction * (newV2 - v2));

            if (ball1.GetSpeed() == 0 && ball2.GetSpeed() == 0) // разлепляем слипвшиеся шары
            {
